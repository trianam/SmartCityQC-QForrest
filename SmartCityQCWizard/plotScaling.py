...
if __name__ == "__main__":
    ...
    for windowSizeY in tqdm(wList):
        ...
        yQsvm.append(runQSVMpl.run(datasetFile, xColumns, yColumns, windowSizeX, windowSizeY, splitSeed, usePrecomputedKernel, parallelizeKernel, singleSplit, circuit, returnMF1))

    for kernel in classicKernels:
        plt.plot(wList, ySvm[kernel], label=f"SVM {kernel}", linestyle=':', marker='.', fillstyle='none')
    plt.plot(wList, yQsvm, label="Q-SVM", linestyle='-', marker='o', fillstyle='full')
    plt.legend()
